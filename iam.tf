resource "aws_iam_role" "cluster_instance_role" {
  description        = "cluster-instance-role-${var.deployment_identifier}-${var.cluster_name}"
...
resource "aws_iam_policy" "cluster_instance_policy" {
  description = "cluster-instance-policy-${var.deployment_identifier}-${var.cluster_name}"
...
resource "aws_iam_policy_attachment" "cluster_instance_policy_attachment" {
  name       = "cluster-instance-policy-attachment-${var.deployment_identifier}-${var.cluster_name}"
  roles      = [aws_iam_role.cluster_instance_role.id]
  policy_arn = aws_iam_policy.cluster_instance_policy.arn
}

resource "aws_iam_instance_profile" "cluster" {
  name = "cluster-instance-profile-${var.deployment_identifier}-${var.cluster_name}"
  path = "/"
  role = aws_iam_role.cluster_instance_role.name
}

resource "aws_iam_role" "cluster_service_role" {
  description        = "cluster-service-role-${var.deployment_identifier}-${var.cluster_name}"
...
resource "aws_iam_policy" "cluster_service_policy" {
  description = "cluster-service-policy-${var.deployment_identifier}-${var.cluster_name}"
